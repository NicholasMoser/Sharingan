...
class MenuManagerTest(unittest.TestCase):
    ...
    def test_menu_manager_press_start_even(self):
        self.state.frame = 0
        self.mm.press_a_lots(self.state, self.pad)
        self.pad.press_button.assert_called_with(p3.pad.Button.A)

    def test_menu_manager_press_start_odd(self):
        self.state.frame = 1
        self.mm.press_a_lots(self.state, self.pad)
        self.pad.release_button.assert_called_with(p3.pad.Button.A)

    def test_menu_manager_pick_itachi_presses(self):
        self.state.players[2].cursor_x = -23.5
        self.state.players[2].cursor_y = 11.5
        self.mm.pick_itachi(self.state, self.pad)
        self.pad.press_button.assert_called_with(p3.pad.Button.A)
        self.assertTrue(self.pad.selected_fox)

    def test_menu_manager_pick_itachi_tilts(self):
    ...
    def test_menu_manager_pick_itachi_plays(self):
        # This doesn't need to be too strict.
        self.mm.selected_itachi = True
        self.state.frame = 0
        self.mm.pick_itachi(self.state, self.pad)
