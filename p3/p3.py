...
def find_dolphin_dir():
    ...
    candidates = ['~/.local/share/dolphin-emu', '~/.config/dolphin-emu', '~/.dolphin-emu', '~/.local/share/.dolphin-emu']
...
def run(itachi, state, sm, mw, pad, stats):
    ...
    while True:
        ...
        if state.frame > last_frame:
            ...
            start = time.time()
            make_action(state, pad, mm, itachi)
            stats.add_thinking_time(time.time() - start)

def make_action(state, pad, mm, itachi):
    itachi.advance(state, pad)
    return
    if state.menu == p3.state.Menu.Game:
        itachi.advance(state, pad)
    elif state.menu == p3.state.Menu.Characters:
        mm.pick_itachi(state, pad)
...
def main():
    ...
    try:
        ...
        with p3.pad.Pad(pad_path) as pad, p3.memory_watcher.MemoryWatcher(mw_path) as mw:
            run(itachi, state, sm, mw, pad, stats)
